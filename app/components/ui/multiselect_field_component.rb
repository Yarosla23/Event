module Ui
  class MultiselectFieldComponent < ViewComponent::Base
    def initialize(form:, field_name:, label:, options:, selected_options: [], input_class: "")
      @form = form
    ...
    def call
      content_tag(:div, class: "field px-3") do
        ...
        multiselect_input = multiselect_field

        error_tag = error_message
        label_tag + multiselect_input + error_tag
      end
    end
    ...
    def multiselect_field
      ...
      @form.select(@field_name, @options, { multiple: true, selected: @selected_options }, class: select_classes)
    end
